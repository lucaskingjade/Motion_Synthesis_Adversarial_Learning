...
def open_file(filename=None):  # read_file file_read load_file
    ...
    if len(filename) > 0:
        ...
        mycamera.t[2] = myskeleton.maxz + 190
        mycamera.yrot = 0
        mycamera.Recompute()
        if mymenu.readout:
            myviewport.draw_readout(mycamera)
...
def redraw(filename=None):
    ...
    print filename
    mycanvas.update()
    mycanvas.postscript(file=filename)
...
if __name__ == "__main__":
    ...
    if mymenu.readout:
        myviewport.draw_readout(mycamera)
    #filename = "./SW_Anxiety_index49SW_Anger_index34_longsequence_frame100.bvh"
    ...
    import io
    jpg_list = ""
    eps_name = filename[:-4]+"_frame1.eps"
    open_file(filename)
    jpg_name = eps_name[:-3]+'png'
    jpg_list = jpg_list +' '+ jpg_name
    command = 'convert ' + eps_name + " " + jpg_name
    ...
    with open(filename,'rb') as fl:
        lines = fl.readlines()
        print lines[137]
        nb_frames = int(lines[137][8:-1]) -2
        print "nb_frame:%d"%nb_frames


    for i in range(nb_frames):
        eps_name = filename[:-4]+"_frame"+str(i+2)+".eps"
        onStepfd()
        jpg_name = eps_name[:-3]+'png'
        command = 'convert '+ eps_name+" "+ jpg_name
        stream = os.popen(command)
    ...
    print '...'
    print '...'
    command = 'convert -delay 0.08333 -dispose previous -loop 0 '+ jpg_list +' '+filename[:-3]+'gif'
    #command = 'convert -antialias -delay 0.08333' + jpg_list + ' ' + filename[:-3] + 'mp4'


    stream = os.popen(command)
    #remove all the eps and jpg files
    stream = os.popen("rm *eps")

    stream = os.popen("rm "+ jpg_list)
    ...
    #plot one frame of the bvh file
     #change the z size as x =299 y=56 z = 418
    #convert to eps format
    #save eps to image
    #save images to gif with a duration same with frames in bvh.
