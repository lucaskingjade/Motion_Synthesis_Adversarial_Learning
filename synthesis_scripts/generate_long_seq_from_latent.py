...
if __name__ =="__main__":
    ...
    ###get a new speed_seq and a motion sequence for generate another latent vector

    speed_seq_2 = speed_xz[indices[index]:indices[index]+7]
    x_seq_2 = X[indices[index]:indices[index]+7]
    print "shape of x_seq_2 is {}".format(x_seq_2.shape)
    # load encoder and decoder
    print 'load encoder ...'
    # with open('./encoder.yaml','r') as f:
    #     encoder = model_from_yaml(f)

    max_len = x_seq_2.shape[-2]
    encoder = encoder(max_len=max_len,dof=dof,
                      hidden_dim_enc_list=[100,100],activation_enc_list=['tanh','tanh'],
                      latent_dim=50,speed_dim=1)
    encoder.load_weights('./encoder.h5')
    if len(x_seq_2.shape) != 3:
        raise ValueError()
    true_latent_vector = encoder.predict(x=[x_seq_2,speed_seq_2])
    print "shape of true_latent_vectors is {}".format(true_latent_vector.shape)
    ...
    for cc in range(num_copy+1):
        ...
        alpha = cc*0.1+alpha
        if cc != num_copy:
            new_speed_seq.extend(speed_seq[:]*alpha)
        else:
    ...
    #new_speed_seq = np.asarray([new_speed_seq,]*20)
    ...
    latent_vector = generate_positive_samples(1,mean,covariance,'Gaussian',seed=np.random.randint(0,2000))
    ...
    output_sequence = decoder.predict(x=[latent_vector,new_speed_seq])

    ## generate from the true latent vector and another new_speed_seq
    new_speed_seq_2 = []
    tmp_speed_seq = new_speed_seq.reshape(new_speed_seq.shape[1],new_speed_seq.shape[2])
    for i in range(len(true_latent_vector)):
        new_speed_seq_2.append(tmp_speed_seq)
    new_speed_seq_2 = np.asarray(new_speed_seq_2)

    print "new_speed_seq shape is {}".format(new_speed_seq_2.shape)
    output_sequence2 = decoder.predict(x=[true_latent_vector,new_speed_seq_2])
    ...
    if args.smooth :
        ...
        output_sequence2 = smooth_sequence(output_sequence2,window_length=41,polyorder=3)
    ...
    print "output_sequence2 shape is {}".format(len(output_sequence2))
    sequence2 = postprocess_seq(output_sequence2, new_speed_seq_2, max_vector=max_vector, min_vector=min_vector,
                               max_vertical_position=max_vertical_position, min_vertical_position=min_vertical_position,
                               actor=1)
    ...
    sequence2 = smooth_sequence(sequence2, window_length=41, polyorder=3)
    #save to file

    print "saving long sequence..."
    for i,seq in enumerate(sequence):
        act1_str = ''.join(c for c in args.activity if c.isupper())
        file_name = './results/'+act1_str + '_' + args.emotion + str(args.index)+'_longsequence_frame' \
                    +str(max_len)+'_'+str(i)+ '.bvh'
        save_seq_2_bvh(seq, file_name)



    for i,seq in enumerate(sequence2):
        act1_str = ''.join(c for c in args.activity if c.isupper())
        file_name = './results/'+act1_str + '_' + args.emotion + str(args.index)+'_longsequence2_frame' \
                    +str(max_len)+'_'+str(i)+ '.bvh'
        save_seq_2_bvh(seq, file_name)
