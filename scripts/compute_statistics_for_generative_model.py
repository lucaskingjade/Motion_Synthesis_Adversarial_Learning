...
if __name__=='__main__':
    ...
    print 'seq2seq = {}'.format(args.seq2seq)
    ...
    #     indices = np.where(Y1==args.remove_label)[0]
    #     X = X[indices]
    ...
    if activity_emotion_condition ==0:
        ...
        generated_seqs = decoder.predict(x=[noise_vectors,Y1[:nb_total]],batch_size=1000)
    elif activity_emotion_condition ==2:
        generated_seqs = decoder.predict(x=[noise_vectors,Y2[:nb_total]],batch_size=1000)


    print "args.query_difference is {}".format(args.query_difference)
    if args.query_difference is not True:
        ...
        #     all_results['all_lld_' + dataset] = pool.map(fun_parallel, sigma_range)
